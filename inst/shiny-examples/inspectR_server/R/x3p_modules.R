    ...
    function(input, output, session) {
      ...
      observeEvent(input$compute_button, {
        ...
        output$text_window <- renderText({
          paste("Processing finished: x3p_flip_y")
        })
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      observeEvent(input$compute_button, {
        ...
        output$text_window <- renderText({
          paste("Processing finished: x3p_sample, m:", isolate(input$ds_num))
        })
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      observeEvent(input$compute_button, {
        ...
        output$text_window <- renderText({
          paste("Processing finished: x3p_m_to_mum")
        })
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      observeEvent(input$compute_button, {
        ...
        output$text_window <- renderText({
          paste("Processing finished: x3p_rotate, angle:", isolate(input$r_angle))
        })
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      observeEvent(input$compute_button, {
        ...
        output$text_window <- renderText({
          message <- capture.output(print(data()$x3p[[1]]))
    ...
    function(input, output, session) {
      observeEvent(input$compute_button, {
        ...
        if(assertthat::has_name(isolate(data()), "x3p")) {
          ...
          show_modal_spinner(spin = "atom", text = "Computing ...")
          ...
          output$text_window <- renderText({
            paste("Finished the computation. Please check if all components needed for investigation are obtained.")
          })
          ...
          output$text_window <- renderText({
            validate(
              ...
              need(data(), message = "Could not proceed: no x3p files found.")
            )
